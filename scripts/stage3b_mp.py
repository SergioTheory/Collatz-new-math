...
def process_chunk(chunk_data):
    chunk, k_step, is_terminal = chunk_data
    cands_by_bucket = {}
    barina_cands = []
    local_deaths = {'corridor': 0, 'S-d': 0, 'mod9': 0, 'odd': 0, 'phase': 0}
    ...
    for state in chunk:
        ...
        for a in range(start_a, 10, 2):
            ...
            score = -bit_diff + score_branch(new_shifts[-40:], fam, is_barina, lam1, lam2, lam3, dip_t)
            sig = y & ((1<<64) - 1)
...
def stage3b_mp():
    ...
    for k_step in range(12, 2800):
        ...
        results = pool.map(process_chunk, [(ch, k_step, is_terminal) for ch in chunks])
        ...
        if k_step in [200, 400, 600, 800, 1000] or k_step % 100 == 0:
            ...
            if beam:
                ...
                mod3_counts = Counter([st[0] % 3 for st in beam])
                total = len(beam)
