...
@njit
def compute_normalized_beta_S_parallel(d, S, M, h_array):
    K = 1 << M
    ...
    return total_real, total_imag, count

def run_normalized_C4():
    print("--- C4 Normalized: Testing Phase Cancellation inside the Layer ---")
    
    # Test a few d values to verify if there's any true decay
    ...
    for d in d_vals:
        ...
        max_norm_beta_global = 0.0
        max_h_global = 0
        max_S_global = 0
        
        t0 = time.time()
        for S in range(S_start, S_end + 1):
            total_real, total_imag, count = compute_normalized_beta_S_parallel(d, S, M, h_array)
            
            if count > 0:
                # NORMALIZE by the count in the layer!
                beta_mags = np.sqrt(total_real**2 + total_imag**2) / count
                
                max_idx = np.argmax(beta_mags)
                max_beta = beta_mags[max_idx]
                max_h = h_array[max_idx]
                
                if max_beta > max_norm_beta_global:
                    max_norm_beta_global = max_beta
                    max_h_global = max_h
                    max_S_global = S
                
        t1 = time.time()
        print(f"  Max Normalized |beta_{{d,M,S}}(h)| = {max_norm_beta_global:.6f} at S={max_S_global}, h={max_h_global}", flush=True)
        print(f"  Time taken: {t1 - t0:.2f} seconds", flush=True)
        results.append((d, max_norm_beta_global))
        
    print("\nSummary of Normalized Maxima:")
    for d, val in results:
        print(f"  d={d:2d}: {val:.6f}")
