...
EXPANDED: k up to 2000, m up to 60 (3x more data)
...
# EXPANDED data range
K_MAX = 2000  # Was 999, now 2000
M_MAX = 60    # Was 40, now 60
...
def remove_low_divisors(trajs: List[Dict], remove: List[int]) -> List[Dict]:
    ...
    for t in trajs:
        new_t = t.copy()
        for i in range(1, 61):  # Up to 60 columns now
            col = f'div_{i}'
...
def extract_high_divisors(trajs: List[Dict], min_div: int) -> Dict[int, List[Dict]]:
    ...
    for t in trajs:
        ...
        high_divs = []
        for i in range(1, 61):  # Up to 60 columns
            col = f'div_{i}'
            if col in t and t[col]:
                d = int(t[col])
                if d >= min_div:
                    high_divs.append({'position': i, 'divisor': d, 'm': int(t['m'])})
...
def create_filtered_table(by_k: Dict[int, List[Dict]], output_file: str, 
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"Range: k=1 to {K_MAX}, m=0 to {M_MAX}\n")
        f.write("=" * 80 + "\n\n")
...
def main():
    print("=" * 80)
    print("GENERATING DATA IN SEPTembrino's FILTERED TABLE FORMAT (EXPANDED)")
    print(f"Range: k=1 to {K_MAX}, m=0 to {M_MAX}")
    print("=" * 80)
    print()
    ...
    print("=" * 80)
    print()
    print("Next step: Run septembrino_matrix_mp.py with K_MAX=2000, M_MAX=60")
    print("Then re-run this script for full data.")
