    ...
    for k_step in range(12, 400):
        ...
        if k_step == 258:
            ...
            with open("expand_913.json", "r") as f:
                ref = json.load(f)
            ref_set = set(int(item['n']) for item in ref)
            found = set()
            for st in beam:
                orig_x = st[0]
                if orig_x in ref_set:
                    found.add(orig_x)
            print(f"Found {len(found)} seeds from expand_913.json!")
            break
