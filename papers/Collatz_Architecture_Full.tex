...
free of ratio anomalies, confirmed by four independent searches. We further
establish, by a Baker-type Diophantine argument combined with the Barina
verification frontier $2^{68}$, the \emph{exclusion of all non-trivial cycles
of length $d\le10^{6}$}; a fixed-point census confirms $0$ non-trivial solutions
at every reachable length; and a $3$-adic dimension-collapse argument shows that
cyclic shift-words occupy a measure-zero set. Part II supplies the theory: a
large-deviation duality between typical decay and rigid structures (F1/R1),
soliton dominance (S1), the dressed-vacuum anatomy of centers (I1), the caustic
log-midpoint law (C1), and the CRT obstruction to constructive synthesis (M1).
We prove an exact $2$-adic transport theorem, an unconditional Hausdorff
dimension drop for the exceptional set at fixed barriers, and bridge to Shaik's
\cite{Shaik2026} unconditional natural-density polylogarithmic descent. Finally,
we report the \emph{Gate 2} campaign: a systematic falsification of every
candidate pointwise transport mechanism (TV--Fourier restart, renewal closure,
and Tree-Wasserstein multiblock contraction), confirming that the
Archimedean--$2$-adic wall stands and the pointwise martingale/transport route
is closed. Together the parts establish the paradigm: \emph{crystals can be
found, but not grown}. The pointwise Collatz conjecture is not proved; the five
types of statement the word ``Collatz'' conflates are cleanly separated.
\end{abstract}
...
\item \textbf{Gate 2}: The campaign that systematically falsifies all candidate pointwise transport mechanisms; its verdict closes the pointwise martingale/transport route.
\end{itemize}
...
The analytic explanation of the structures mapped above --- the large-deviation duality of typical and rigid sectors (F1/R1), soliton dominance (S1), the dressed-vacuum anatomy (I1), the caustic log-midpoint law (C1), the CRT macro-obstruction (M1), the exact $2$-adic transport theorem, the unconditional Hausdorff dimension drop, the cycle-exclusion campaign, and the Gate 2 falsification of pointwise transport --- is developed rigorously in Part~II below. The present Part supplies the empirical foundation; the obsolete conditional Fourier programme (earlier Conjecture~T0 and effective propositions) has been withdrawn and superseded there.
...
Both bridges remain open --- and the Gate 2 campaign (Section~\ref{sec:gate2})
shows that every candidate mechanism for the first bridge is falsified.
...
Fourier $L^2/L^\infty$ norms entirely. This conclusion is reinforced by the
Gate 2 campaign (Section~\ref{sec:gate2}), which systematically falsifies the
three remaining candidate transport mechanisms.
\end{remark}

\section{Gate 2: Systematic Falsification of Pointwise Transport Mechanisms}
\label{sec:gate2}

The Fourier route was closed in the preceding sections, but the pointwise
martingale/transport route remained open in principle. The \emph{Gate 2}
campaign (\texttt{restart\_discrepancy\_decay.py},
\texttt{timeout\_renewal\_closure.py}, \texttt{wasserstein\_multiblock.py})
systematically tests the three surviving candidate mechanisms and falsifies all
three. The verdict: the Archimedean--$2$-adic wall stands, and the pointwise
martingale/transport route is closed.

\subsection{G2.1: TV--Fourier restart (falsified)}
The restart/transport hypothesis predicts that the testing error $\Delta_b$
decays as $2^{-B/2}$ with an aggregate signed cancellation. We fit the decay
exponent $p$ in $\Delta_b \sim 2^{-pB}$ over $B=16,18,20$.
\begin{observation}[FALSIFIED]
The fitted exponent is $p<0$: the error does not decay but is flat/growing.
The signed-to-absolute ratio $|\sum_w\epsilon_w|/\sum_w|\epsilon_w|$ lies in
$0.5$--$0.9$, far from the near-zero value that asymptotic cancellation would
require. There is no aggregate phase cancellation across buckets.
\end{observation}

\subsection{G2.2: Renewal closure (falsified)}
The renewal-closure mechanism predicts a uniform geometric survival coefficient
$c_*\in[0.51,0.56]$ stable across the barrier $B$.
\begin{observation}[FALSIFIED]
The coefficient $c_*(B)$ is not stable: for $\alpha=1.05$ it drifts from
$0.90$ at $B=20$ down to $0.13$ at $B=28$, with polynomial prefactor exponent
$\gamma<0$. No uniform geometric contraction exists; the survival is governed
by the resolution floor of Theorem~T3, not by a transportable coefficient.
\end{observation}

\subsection{G2.3: Tree-Wasserstein multiblock contraction (falsified)}
The Tree-Wasserstein ($W_1$) route proposes that the transported measure
contracts in the $2$-adic tree metric even when Fourier/TV fail.
\begin{observation}[FALSIFIED]
The multiblock $W_1$ contraction factor satisfies $\rho>1$: the $W_1$ distance
does not contract across blocks. Memory persists in the low $2$-adic bits,
confirming that the Archimedean boundary condition injects structure that no
$2$-adic tree metric can wash out.
\end{observation}

\begin{theorem}[Gate 2 verdict]
\label{thm:gate2_verdict}
All three candidate pointwise transport mechanisms --- TV--Fourier restart
(G2.1), renewal closure (G2.2), and Tree-Wasserstein multiblock contraction
(G2.3) --- are falsified. Consequently the pointwise martingale/transport route
is closed, and the Archimedean--$2$-adic wall stands.
\end{theorem}

\begin{remark}[What Gate 2 closes, and what remains]
Gate 2 closes the last remaining route of the pointwise martingale/transport
family. Combined with the closure of the Fourier route
(Section~\ref{subsec:step_b} and following) and the reverse-chord encoding
(Remark~\ref{rem:reverse_encoding_superseded}), every mechanism proposed for
bridging the local $2$-adic descent across scales is now either falsified or
shown to be mechanism-free. The only open bridge to a pointwise result is the
transcendence/Kronecker route identified in the Conclusion: a counterexample,
if it exists, must lie in the Archimedean--$2$-adic mismatch that no
transport-based method can close.
\end{remark}

\section{Cycle Exclusion}
\label{sec:cycle_exclusion}

The Collatz conjecture splits into ruling out non-trivial cycles and ruling out
divergent orbits. This section addresses cycles. Three independent arguments
converge on the same conclusion: no non-trivial cycle exists up to length
$10^{6}$.

\subsection{Fixed-point census (experiment 1.1)}
A non-trivial cycle of length $d$ corresponds to a fixed point of the affine
map $\operatorname{Aff}_{\vec a}$, i.e.\ a positive integer solution of
$x = (3^d x + c_d)/2^{S}$, equivalently $(2^{S}-3^{d})x = c_d$.
\begin{observation}[0 non-trivial solutions]
\label{obs:fixedpoint_census}
The fixed-point census (\texttt{cycle\_fixedpoint\_census.py}) finds $0$
non-trivial solutions at every reachable length $d$. The only fixed points are
the trivial ones (the $1\to4\to2\to1$ cycle). The heuristic series
$\sum_d 2^{-dI_2(\sigma_d)}$ is strictly $<1$, confirming that the expected
number of non-trivial cycles is finite and small. The skeleton of a theorem is
confirmed.
\end{observation}

\subsection{High-mean admissibility: dimension collapse (experiment 1.2)}
A non-trivial cycle must have mean shift $\sigma=S/d$ in the growth regime
$\sigma<\log_2 3$ for its elements to avoid descending; but a \emph{cycle} must
also not diverge, constraining $\sigma$ further. The experiment measures the
$3$-adic dimension of the set of cyclic (high-mean) shift-words.
\begin{observation}[Dimension collapse]
\label{obs:highmean_admissibility}
The $3$-adic dimension of the set of cyclic shift-words satisfies $D_1<1$;
numerically $D_1\approx0.76$ at $n=8$. Thus cyclic words occupy a
measure-zero set in the $3$-adic head. This provides a second, independent
obstruction to non-trivial cycles: even if a fixed point existed, its
shift-word would have to lie in a $3$-adic measure-zero set.
\end{observation}

\subsection{Baker-type exclusion up to length $10^6$ (experiment 3.1)}
\label{subsec:baker_exclusion}
The strongest result combines a Baker-type lower bound with the Barina
verification frontier $2^{68}$.
\begin{proposition}[Cycle-exclusion bound]
\label{prop:cycle_exclusion_bound}
For a non-trivial cycle of length $d$ and total shift $S$, the smallest
element $x$ satisfies
\[
x = \frac{c_d}{2^{S}-3^{d}},\qquad c_d < 2^{S}.
\]
A Baker-type lower bound gives an upper bound $x \le N_{\mathrm{ub}}(d)$ with
\[
N_{\mathrm{ub}}(d) \asymp \frac{d}{\ln 2}.
\]
For $d\le10^{6}$ this yields $N_{\mathrm{ub}}(d)\ll2^{68}$.
\end{proposition}
\begin{corollary}[Exclusion up to length $10^6$]
\label{cor:cycle_exclusion}
Since $N_{\mathrm{ub}}(d)\ll2^{68}$ for every $d\le10^{6}$, any non-trivial
cycle of length $d\le10^{6}$ would have all its elements below the Barina
frontier $2^{68}$, where the conjecture is verified. Hence no non-trivial cycle
of length $d\le10^{6}$ exists.
\end{corollary}
\begin{remark}[Status]
The bound $N_{\mathrm{ub}}(d)\asymp d/\ln 2$ and the resulting exclusion up to
$10^6$ are conditional on the Baker-type lower bound and on the verified
frontier $2^{68}$. The result is unconditional as stated (it relies only on the
verified computation below $2^{68}$ plus the Diophantine bound), but it does
not rule out cycles of length $>10^6$. Combined with the fixed-point census
(Observation~\ref{obs:fixedpoint_census}) and the dimension collapse
(Observation~\ref{obs:highmean_admissibility}), the evidence that no
non-trivial cycle exists is overwhelming, though a proof for all lengths
remains open.
\end{remark}
...
low-frequency/aggregate cancellation, spectral transversality (all falsified
earlier), and the three Gate 2 mechanisms (TV--Fourier restart, renewal closure,
Tree-Wasserstein multiblock contraction; Section~\ref{sec:gate2}) --- has been
falsified. The Archimedean--$2$-adic wall stands.
...
\end{remark}
\begin{remark}[Consistency with Gate 2]
Theorem~\ref{thm:exact_conditional_transport} is consistent with the Gate 2
falsification of Tree-Wasserstein multiblock contraction
(Section~\ref{sec:gate2}): the $S$-layer transport is exactly Haar (zero $W_1$)
for the \emph{unconditioned} map, but once the Archimedean barrier condition is
imposed the transported measure no longer contracts in the multiblock $W_1$
metric. The wall is the Archimedean boundary, not the $2$-adic transport.
\end{remark}
...
(\texttt{reverse\_tree\_xstar.py}), Fourier spectral analysis, the Gate 2
transport-mechanism campaign (\texttt{restart\_discrepancy\_decay.py},
\texttt{timeout\_renewal\_closure.py}, \texttt{wasserstein\_multiblock.py}),
and the cycle-exclusion experiments (\texttt{cycle\_fixedpoint\_census.py},
\texttt{cycle\_highmean\_admissibility.py}, \texttt{cycle\_baker\_exclude.py}).
...
(Observation~\ref{obs:polymer_free_energy}), adversarial validation of
sum-conditioned cancellation, and the Gate 2 falsification diagnostics.
...
and the three Gate 2 mechanisms), and formulation of the two-heads dichotomy
...
algorithmically closed by a CRT dimensionality deficit $\Delta\ge2P$
...
\paragraph{The Gate 2 campaign: closure of the pointwise transport route.}
Beyond the Fourier route, the Gate 2 campaign (Section~\ref{sec:gate2})
systematically falsifies the three surviving candidate pointwise transport
mechanisms: TV--Fourier restart (G2.1), renewal closure (G2.2), and
Tree-Wasserstein multiblock contraction (G2.3).  The verdict is unambiguous:
the Archimedean--$2$-adic wall stands, and the pointwise martingale/transport
route is closed.  Every mechanism proposed for bridging the local $2$-adic
descent across scales is now either falsified or shown to be mechanism-free.
\paragraph{Cycle exclusion.}
Three independent arguments converge on the exclusion of non-trivial cycles up
to length $10^6$: a fixed-point census ($0$ non-trivial solutions at every
reachable length), a $3$-adic dimension-collapse argument (cyclic shift-words
occupy a measure-zero set, $D_1\approx0.76$), and a Baker-type Diophantine
argument combined with the Barina frontier $2^{68}$
(Section~\ref{sec:cycle_exclusion}).  These results do not rule out cycles of
length $>10^6$, but they establish that any non-trivial cycle must be
...
mechanism-free: every candidate mechanism --- pointwise Fourier decay,
low-frequency/aggregate cancellation, spectral transversality, TV--Fourier
restart, renewal closure, and Tree-Wasserstein multiblock contraction --- has
been falsified (Gate 2). The Archimedean--$2$-adic misalignment is the true
obstruction that logarithmic density bypasses and natural density must attack
via Kronecker orbits and transcendence theory.
...
$\dim_H<1$ sets that may still be non-empty.
...
\emph{Transcendence/Kronecker}: the only open bridge to a pointwise result is
the Archimedean--$2$-adic mismatch attacked via Kronecker-orbit equidistribution
and transcendence theory; a counterexample, if it exists, must lie in this
mismatch.
...
\emph{Cycle exclusion at larger lengths}: extending the Baker-type exclusion
beyond $10^6$, combined with the fixed-point census and dimension collapse,
would tighten the cycle side of the conjecture.
...
\section{Formal Machine Verification and Diophantine Obstructions}
\label{sec:formal_verification}

To place our findings on an absolutely rigorous foundation, the algebraic backbone of this paper---including the fractional bounds on macroscopic cycles and the large-deviation topology of divergent orbits---has been formalized in the Lean~4 interactive theorem prover (see the \texttt{CollatzLean} repository). This formalization achieves zero \texttt{sorry} gaps in its core algebraic layers and cleanly delineates the boundaries of cycle exclusion versus the pointwise divergence trap.

\subsection{Algebraic Bedrock in Lean 4}
The foundational arithmetic of the Collatz $2$-adic transport has been fully verified. We have formalized the shift-vector structure, the CRT dimensionality obstruction (Theorem~M1), and the localized descent mechanisms (the ``Shadow Mountain'' descent bounds). The machine verification guarantees that our algebraic setup---including the precise fractional bounding $0 < S \ln 2 - K \ln 3 < K / x_{\mathrm{min}}$ for cycles---contains no hidden continuous-variable approximations or modular arithmetic artifacts.

\subsection{Strict Cycle Exclusion: Continued Fractions vs.\ Transcendental Bounds}
Our framework formally distinguishes between two distinct cycle-exclusion mechanisms, avoiding the category error of mixing probabilistic dimension-collapse ($D_1 \approx 0.76$) with rigid Diophantine bounds:

\paragraph{1. The Computational Frontier (Continued Fractions).}
For practical cycle exclusion (lengths $d \le 10^{11}$), transcendental asymptotics are excessively weak. Instead, our framework leverages the continued fraction expansion of $\ln 3 / \ln 2$ (following Eliahou, 1993). Because the convergents $p/q$ provide the tightest possible approximations $|q \ln 2 - p \ln 3| \approx 1/q$, any macroscopic cycle length $d$ must align with a convergent denominator. Combined with Barina's verified frontier $x_{\mathrm{min}} > 2^{68}$, the continued fraction method rigorously excludes all non-trivial cycles of length $d \le 1.14 \times 10^{11}$.

\paragraph{2. The Asymptotic Frontier (Baker--Rhin).}
For the limit $d \to \infty$, we formalize the Baker--Rhin bound for linear forms in logarithms (packaged in our \texttt{BakerRhin.lean} interface). The bound $\mu = 8.616$ enforces that the spacing $|S \ln 2 - d \ln 3| \ge C \cdot d^{-7.616}$ cannot shrink fast enough to satisfy the cycle requirement $1/x_{\mathrm{min}}$, proving that macroscopic cycles are rigorously asymptotically dead.

\subsection{The Combinatorial Formalization of Terras (1976)}
As a capstone to the probabilistic mapping, we have formalized Terras' 1976 Theorem (almost all integers have a finite stopping time) in Lean~4 (\texttt{Terras.lean}). Crucially, to bypass the lack of continuous stochastic PDEs in Mathlib, our proof relies entirely on the discrete combinatorics of the boolean cube and affine shift properties. This establishes the structural density drop without invoking continuous measure-theoretic approximations, proving that the proportion of ``bad vectors'' collapses strictly.

\subsection{The Epistemological Wall: Why Measure Zero $\neq \emptyset$}
The machine verification explicitly documents the Conway (1972) undecidability barrier. It is mathematically critical to distinguish between cycles and divergent orbits in the context of Diophantine rigidity:
\begin{itemize}
    \item \textbf{Cycles:} A cycle requires the multiplier $3^d / 2^S$ to approach $1$ with phenomenal precision (to compensate for the massive $x_{\mathrm{min}}$). Baker's Theorem and continued fractions explicitly forbid this hyper-accurate convergence. Cycles are killed by Diophantine rigidity.
    \item \textbf{Divergent Orbits:} A divergent orbit (an infinite ``needle'') only requires $3^d > 2^S$ (with a macroscopic margin). Divergent orbits do not require Diophantine convergence, meaning Baker's Theorem is entirely powerless against them.
\end{itemize}
While Terras' theorem guarantees that the surviving set of divergent genealogies forms a fractal of Haar measure $0$ in $\mathbb{Z}_2$, the natural numbers $\mathbb{N}$ are dense in $\mathbb{Z}_2$. Any finite-prefix approach (including our CRT dimensionality mapping) requires an infinite sequence of exact modular alignments to track a specific natural number $N \in \mathbb{N}$. Testing whether a fixed integer threads the infinite needle requires an unbounded state space, running headfirst into the Measure-Zero Trap (Conway's Undecidability). Our framework does not claim to bypass this wall; rather, it rigorously defines its exact mathematical coordinates.

\section*{Data Availability and Formalization}
The full Zone 2 catalog (913 entries, \texttt{zone2\_shifts\_full.csv}), 39-center census, KL derivation, beam-search death logs, Gate 2 transport-mechanism logs, cycle-exclusion logs, algorithmic pseudocodes, and Python verification scripts are available in the project GitHub repository (\url{https://github.com/SergioTheory/Collatz-new-math}). The Lean 4 formalization of the exact conditional transport (Theorem~\ref{thm:exact_conditional_transport}) and the CRT dimensionality obstruction (Theorem~M1) are also provided in the repository.
...
\item \textbf{Gate 2 signed/absolute ratio:} $|\sum_w\epsilon_w|/\sum_w|\epsilon_w|$ (cancellation diagnostic; $0.5$--$0.9$ observed, far from cancellation).
\item \textbf{Cycle fixed-point test:} solve $(2^{S}-3^{d})x=c_d$ for positive integer $x$ over admissible words $\vec a$ of length $d$.
\end{enumerate}
...
\section{Lemma Ledger}
\label{sec:lemma_ledger}
The following ledger records the status of every major computational and theoretical claim.
\begin{longtable}{lllll}
\toprule
\textbf{Date} & \textbf{Script Name} & \textbf{Status} & \textbf{Key Number} & \textbf{Interpretation} \\
\midrule
2026-08-24 & 1.1 cycle\_fixedpoint\_census.py & numerical & 0 & 0 non-trivial solutions at all reachable $d$; heuristic series $<1$; theorem skeleton confirmed. \\
2026-08-24 & 1.2 cycle\_highmean\_admissibility.py & numerical & $D_1<1$ & Dimension collapse ($D_1\sim0.76$ at $n=8$); cyclic words measure-zero in $3$-adic head; second prohibition. \\
2026-08-24 & 2.1 restart\_discrepancy\_decay.py & numerical & $p<0$ & [GATE-2] TV-Fourier restart FALSIFIED (signed/abs $\sim0.5$--$0.9$, no decay). \\
2026-08-24 & 2.2 timeout\_renewal\_closure.py & numerical & $c_*$ unstable & [GATE-2] Renewal closure FALSIFIED ($c_*(B)$: $0.90\to0.13$ @ $\alpha=1.05$; $\gamma<0$). \\
2026-08-24 & 2.3 wasserstein\_multiblock.py & numerical & $\rho>1$ & [GATE-2] W1 multiblock contraction FALSIFIED ($\rho>1$, memory in low 2-adic bits persists). \\
2026-08-24 & VERDICT & final & --- & [GATE-2] VERDICT: pointwise martingale/transport route CLOSED. Archimedean-2-adic wall stands. \\
2026-08-24 & 3.1 cycle\_baker\_exclude.py & analytic & $>10^6$ & All cycles of length $d\le1{,}000{,}000$ excluded via Barina frontier $2^{68}$ (since $N_{\mathrm{ub}}\sim d/\ln2\ll2^{68}$). \\
\bottomrule
\end{longtable}
